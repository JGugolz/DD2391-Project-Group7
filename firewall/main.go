...
var (
	tcpTable          = NewStateTable()
	stateTableEnabled = true // Set to false to disable TCP state table filtering
)
	...
	fn := func(a nfqueue.Attribute) int {
		...
		if ip, tcp, plen, ok := decodeIPv4TCPWithPayload(data); ok {
			if stateTableEnabled {
				// STATE TABLE ENABLED - Apply all protections
				...
				// STATE TABLE DISABLED - Accept all packets, just log them
				log.Printf("ACCEPT (state table disabled) %s:%d -> %s:%d flags[S:%t A:%t F:%t R:%t]",
					ip.SrcIP, tcp.SrcPort, ip.DstIP, tcp.DstPort,
					tcp.SYN, tcp.ACK, tcp.FIN, tcp.RST,
				)
			}
